...
final class StructuredLogger
{
    ...
    private static function safeStringForKey(int|string $key, string $value): string
    {
        ...
        $allowed = match ($key) {
            ...
            'route' => preg_match('/\A(?:bootstrap|unknown|login|sso|logout|portal|modules|public|student|board|move|return-to-idle|board-preferences|notifications|notification-acknowledge|candidate|records(?:-(?:candidate|company|round|schedule|panelist|slot-assignment|application))?|operations|reports|import(?:-rollback)?|admin(?:-(?:user|users|password|workflow))?|integrations|integration-(?:create|secret-(?:generate|rotate)|validate|activate|disable|revoke|replay)|preferences(?:-resolve)?|wanted(?:-resolve)?|system(?:-clear-demo)?|advising(?:-(?:appointment|status|note|task))?|api-v1-(?:root|unknown|opportunities-(?:list|item)|applications-(?:list|item|transition)))\z/D', $value) === 1,
