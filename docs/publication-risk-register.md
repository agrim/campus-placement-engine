...
| Legacy SQL dumps expose passwords, emails, phone numbers, or operational rows | Critical | SQL dumps remain under `.legacy-private/` or ignored archive patterns; only reviewed data-plane and control-plane migration SQL is allowed | Never publish SQL dumps; document schema through migrations only |
...
| Placement-day operators act on stale screens or duplicate submits | High | Stale-board guard, idempotency keys, persisted stale thresholds, board-only HTML refresh, readiness checks | Continue browser QA with the generated refresh and stale-screen checklist |
